...
const newArr = workers.map((emp) => {
  return { nom: emp.name, salary: emp.salary };
});
...
const earning = workers.every((w) => w.salary > 30000);
...
searchBar.addEventListener("input", () => {
  ...
  const filtering = workers.filter(
    (w) =>
...
const newHires = [{ name: "Frank Hardy" }, { name: "Grace Wilson" }];
...
// Save workers array to localStorage
const div31 = document.createElement("div");
const buutn = document.createElement("button");

buutn.textContent = "Save to LocalStorage!";

buutn.addEventListener("click", () => {
  const save = localStorage.setItem("workers", JSON.stringify(workers));
  div31.innerHTML = `<p>Workers saved to localStorage!</p>`;
});

div31.appendChild(buutn);
all.appendChild(div31);

// Load from LocalStorage
const div32 = document.createElement("div");
const buuutn = document.createElement("button");

buuutn.textContent = "Load from LocalStorage!";

buuutn.addEventListener("click", () => {
  const save = JSON.parse(localStorage.getItem("workers"));
  ...
  if (save.length > 0) {
    ...
    save.forEach((worker) => {
      const p = document.createElement("p");
      p.textContent = `${worker.name}`;
      div32.appendChild(p);
    });
  }
});

div32.appendChild(buuutn);
all.appendChild(div32);
